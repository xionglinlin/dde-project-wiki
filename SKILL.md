...
description: |
  ...
  - 用户要导入历史文档 ("导入这些分析", "把这些文档加入 wiki", "导入历史记录", "把之前的分析导入")
...
**触发词**："导入这些分析"、"把这些文档加入 wiki"、"导入历史记录"、"把之前的分析导入"，或直接指定目录/文件路径。

**工作流程**：

1. **读取文档**：读取用户指定目录或文件的所有 .md 文件

2. **智能分类**（按内容特征关键词）：
   - 包含 "bug"、"问题"、"修复"、"PMS"、"崩溃"、"异常" → `bug-analysis/`
   - 包含 "架构"、"流程"、"模块"、"组件"、"机制"、"设计" → `concepts/`
   - 包含 "类"、"接口"、"API"、"服务"、"控制器" → `entities/`
   - 包含 "分析"、"总结"、"对比"、"研究" → `queries/`
   - 无法判断 → 询问用户

3. **转换格式**：
   - 添加标准 YAML frontmatter
   - 从内容中提取 `related_files`（识别源文件路径如 `src/xxx.cpp`）
   - 提取 PMS 编号（如有）
   - 设置 `created` 为原文档日期或导入日期

4. **创建 wiki 页面**：按分类写入对应目录，保留原文档内容

5. **更新交叉引用**：检查涉及已有 entity/concept，添加 wikilinks，更新 `index.md`

6. **追加到 log.md**：
   ```markdown
   ## [YYYY-MM-DD] migrate | 导入历史分析文档
   - 来源：/path/to/old/docs/
   - 导入文档数：5
   - bug-analysis: 3 篇
   - concepts: 1 篇
